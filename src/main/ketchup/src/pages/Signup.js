...
export default function Signup() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const userId = credentials.user.uid; //Firebase Auth를 통해 생성된 사용자 고유 ID를 가져옴
      await set(ref(database, `Users/${userId}`), {
        //Firebase Realtime Database에 데이터를 저장
        id: userId,
        email: email,
        nickname: name,
        profile: "http://via.placeholder.com/160",
      });
  ...
  return (
    <Wrapper>
      ...
      <Switcher>
        Already have an account? <Link to="/">Log in &rarr;</Link>
      </Switcher>
